...
export function buildSessionSnapshot({ session, round, players, boxes, playerId }) {
  ...
  return {
    ...
    readyCheckStartedAt: round.readyCheckStartedAt,
    readyCheckDeadlineAt: round.readyCheckDeadlineAt,
    readyPlayerCount: round.readyPlayerIdsForRound?.length || 0,
    expectedReadyPlayerCount: round.gatePlayerIdsForRound?.length || 0,
    serverTime: Date.now(),
    ...
    you: player
        ? {
          ...
          isReadyForRound: !!player.isReadyForRound,
